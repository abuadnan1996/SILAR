...
unsigned int dipTime;
unsigned int dipLength;
unsigned int withdrawalSpeed;
unsigned int usPerWithdrawalStep;
unsigned int rinseTime;
byte stirrerOn[2];

const uint8_t numOfSettings = 10, numPages = 4;
...
void setup() {
  //  stepperY.stepsPerMicro(-4000, 2500);
  //  while(1);
  Serial.begin(9600);
...
void loop() {
  ...
  uint8_t mode_choice = EEPROM.read(20);
  if (mode_choice == 1) {
    stirrerMode();
  }
...
void stirrerMode() {
  uint8_t st1_speed = EEPROM.read(18);
  uint8_t st2_speed = EEPROM.read(19);
  ...
  while (option != 2)
  {
    ...
    switch (option) {
      ...
      case 1:
        ...
        EEPROM.write(19, st2_speed);
        analogWrite(stirrer2Pin, map(st2_speed, 0, 100, 0, 255));
        break;
      case 2: EEPROM.write(20, 0); analogWrite(stirrer1Pin, 0); analogWrite(stirrer2Pin, 0); break;
    }
  }
}
